import React from 'react'

const DifferenceSection2 = () => {
  return (
    <div>
      <section className="shared-section section-difference section-difference-2">
                <div className="container">
                    <div className="layout">
                        <div className="content-difference order-md-last">
                            <h3 className="title-difference js-reveal-on-scroll js-split-to-lines">Trusted Partner</h3>
                            <p className="paragraph-difference js-reveal-on-scroll js-split-to-lines">Our team is committed to delivering positive, measurable outcomes for all stakeholders – our valued team members, customers, partners, and investors – by operating through our values and standing true to our mission: To make people sure of themselves and organizations sure of their people.®</p>
                        </div>
                        <div className="media js-reveal-on-scroll js-fade-in-up">
                            <div className="lottie-tab-container-difference">
                                <img className="lottie-placeholder" src="img/difference/stack/TrustedPartner.svg" alt="" />
                            </div>
                        </div>
                    </div>
                </div>
            </section>

    </div>
  )
}

export default DifferenceSection2
